...
const TABS = [
  ...
  { value: 'parking',   label: 'Parking',     icon: '🅿️', color: '#64748b' },
...
// ── Mapping type réservation → catégorie budget ──────────────
const BOOKING_TO_BUDGET_CAT = {
  flight:    'flight',
  hotel:     'accommodation',
  car:       'transport',
  train:     'transport',
...
// ── Mapping type → clé dans BOOKING_LINKS ───────────────────
const LINKS_KEY = {
  flight:  'flights',
  hotel:   'hotels',
  car:     'cars',
...
const FIELDS_BY_TYPE = {
  ...
  parking: [
    { key:'confirmation', label:'N° de réservation',         placeholder:'Ex: PK-123' },
    { key:'name',         label:'Nom du parking',            placeholder:'Parking Aéroport T2', required:true },
  ...
  activity: [
    ...
    { key:'price',        label:'Prix (€)',                  type:'number', placeholder:'0.00' },
...
// ── Champ lien avec menu déroulant (générique) ───────────────
const QuickLinkField = ({ value, onChange, hasErr, bookingType }) => {
  const linksKey = LINKS_KEY[bookingType]
  const URLS     = linksKey ? (BOOKING_LINKS[linksKey] || []) : []
  const isKnown  = URLS.some(c => c.url === value)
  ...
  const selectedItem = URLS.find(c => c.url === value)

  if (!URLS.length) {
    return (
      <input type="text" value={value} onChange={e => onChange(e.target.value)}
        placeholder="https://…" style={{ ...baseStyle, cursor: 'text' }} />
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '.4rem' }}>
      ...
      >
        ...
        {URLS.map(c => (
          <option key={c.name} value={c.url}>{c.icon} {c.name}</option>
        ))}
        <option value="__custom__">✏️ Autre (saisir manuellement)</option>
      </select>

      {value && !custom && selectedItem && (
        <a href={value} target="_blank" rel="noopener noreferrer"
          style={{ display: 'inline-flex', alignItems: 'center', gap: '.4rem', padding: '.45rem .9rem', borderRadius: 10, background: 'var(--color-bg-input)', border: '1px solid var(--color-border)', fontSize: '.82rem', fontWeight: 600, color: 'var(--color-primary)', textDecoration: 'none', alignSelf: 'flex-start', transition: 'border-color .15s' }}
          onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--color-primary)'}
          onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--color-border)'}
        >
          🔗 Ouvrir {selectedItem.name}
        </a>
      )}
...
const BookingForm = ({ type, initial, onSave, onCancel }) => {
  ...
  const save = () => {
    ...
    fields.filter(f => f.required).forEach(f => {
      if (!form[f.key]?.trim()) e[f.key] = 'Champ obligatoire'
    })
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1rem 1.25rem' }}>
          {fields.map(f => (
            <div key={f.key} style={{ marginBottom: '.75rem' }}>
              ...
              {f.type === 'quicklink' ? (
                <QuickLinkField value={form[f.key]} onChange={v => set(f.key, v)} hasErr={!!errors[f.key]} bookingType={type} />
              ) : f.multiline ? (
...
// ── Carte réservation ────────────────────────────────────────
const BookingCard = ({ booking, onEdit, onDelete, onAddToBudget }) => {
  ...
  const hasPrice = booking.price && parseFloat(booking.price) > 0

  return (
    <div style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 14, padding: '.85rem', marginBottom: '.55rem', borderLeft: `3px solid ${tab.color}`, transition: 'box-shadow .15s' }}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {hasPrice && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem', marginTop: '.4rem' }}>
              <span style={{ fontSize: '.8rem', fontWeight: 700, color: '#10b981' }}>
                💶 {parseFloat(booking.price).toFixed(2)} €
              </span>
              {booking.addedToBudget ? (
                <span style={{ fontSize: '.68rem', fontWeight: 600, padding: '.2rem .55rem', borderRadius: 20, border: '1px solid #10b98155', background: '#10b98115', color: '#10b981' }}>
                  ✅ Ajouté au budget
                </span>
              ) : (
                <button
                  onClick={() => onAddToBudget(booking)}
                  style={{ fontSize: '.68rem', fontWeight: 600, padding: '.2rem .55rem', borderRadius: 20, border: '1px solid #10b98155', background: '#10b98115', color: '#10b981', cursor: 'pointer' }}
                >
                  ➕ Ajouter au budget
                </button>
              )}
          ...
          {booking.link && (
            <a href={booking.link} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
...
const QuickLinks = ({ type }) => {
  const linksMap = { flight: 'flights', hotel: 'hotels', car: 'cars', train: 'trains', parking: 'parking' }
...
const Bookings = () => {
  ...
  // ── Ajouter une réservation au budget ──────────────────────
  const handleAddToBudget = (booking) => {
    const tab      = TABS.find(t => t.value === booking.type)
    const category = BOOKING_TO_BUDGET_CAT[booking.type] || 'misc'
    const label    = booking.name || booking.company || tab?.label || 'Réservation'
    const date     = booking.departDate || booking.checkIn || booking.pickupDate
    ...
    addExpense({
      ...
      category,
      date,
      note: `Importé depuis ${tab?.label} — ${booking.confirmation || ''}`.trim(),
    })
    // ← Marquer la réservation comme ajoutée au budget
    updateBooking(booking.id, { addedToBudget: true })
    showToast(`💶 ${label} ajouté au budget`)
  }
  ...
  return (
    <div>
        ...
        {TABS.map(t => {
          ...
          return (
            <button key={t.value} onClick={() => setActiveTab(t.value)} style={{
              ...
              {t.icon} {t.label}
              {count > 0 && (
                <span style={{ fontSize: '.65rem', fontWeight: 700, padding: '.1rem .4rem', borderRadius: 20, background: active ? t.color : 'var(--color-bg-input)', color: active ? '#fff' : 'var(--color-text-muted)' }}>
                  {count}
                </span>
              )}
            </button>
      ...
      <div style={{ animation: 'fadeUp .35s .1s ease both' }}>
        {bookingsForTab.length === 0 ? (
          ...
          bookingsForTab.map(b => (
            <BookingCard key={b.id} booking={b} onEdit={handleEdit} onDelete={handleDelete} onAddToBudget={handleAddToBudget} />
          ))
        )}
      </div>
      ...
      {/* Formulaire */}
      {showForm && (
        <BookingForm type={activeTab} initial={editTarget} onSave={handleSave} onCancel={() => { setShowForm(false); setEditTarget(null) }} />
      )}
    </div>
  )
}
